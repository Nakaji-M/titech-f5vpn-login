...
class DarwinPlatform(Platform):
    ...
    def setup_route(self, ifname, gateway_ip, net, bits, action):
        # macOS `route` prefers `-host <ip>` for /32s; using `-net ip/32` works
        # inconsistently and, more importantly, using `-interface <if>` for a
        # non-local VPN gateway target can break connectivity.
        if bits == 32:
            args = [self.route_path, action, '-host', str(net)]
        else:
        ...
        if ifname:
            ...
            # For deletes, specifying the gateway is optional and can cause
            # spurious failures if the kernel does not consider it part of the
            # key.
            if action == 'add':
                args += [gateway_ip]
        run_as_root(args)
...
""" % (params['Session_ID'], params['Session_ID'])
    ...
    if override_gateway:
        ...
        default_route = os.popen(r"netstat -rn | grep -E '^(default|0\.0\.0\.0)' | head -n1").read().split()
        ...
        try:
            # Route the VPN server via the *current* gateway so that when pppd
            # installs a default route, the SSL tunnel does not get sucked into
            # the PPP interface.
            platform.setup_route(None, gw_ip, tunnel_ip, 32, 'delete')
        except:
            pass
        platform.setup_route(None, gw_ip, tunnel_ip, 32, 'add')
    ...
    def ppp_ip_up(iface_name, tty, local_ip, remote_ip):
        ...
        if params.get('LAN0'):
            if not skip_routes:
                if getattr(platform, 'wait_for_interface', None):
                    pid = os.fork()
                    if pid != 0:
                        # Become root
                        os.seteuid(0)
